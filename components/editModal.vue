            ...
            <div class="flex flex-col relative top-[10%] h-[80%] pb-6 bg-white rounded-lg shadow" @click.stop>
                <div class="flex justify-between items-start p-4 rounded-t border-b">
                    <h3 class="text-xl font-semibold text-gray-900">
                        Редактировать персонажа - {{character.name}}
                    </h3>
                ...
                <div class="p-6 overflow-y-scroll scrollarea">
                    <form class="space-y-6" @submit.prevent="submit">
                        <AddInput :value="character.name" text="Название персонажа" tag='name' placeholder="Кошмар"/>
                        <AddInput :value="character.preview" text="Ссылка на иконку" tag='icon' placeholder="https://i.imgur.com/bJUoGF0.png"/>
                        <AddInput :value="character.difficulty" text="Сложность" variant="slider" tag="difficulty" />
                        <AddInput :value="character.image" text="Изображение героя" tag='image' placeholder="https://i.imgur.com/bJUoGF0.png"/>
                        <div class="flex flex-col">
                            <h2 class="text-md font-medium text-gray-900">Таланты</h2>
                            <AddInput :value="character.talents[0]" tag='talent-1' placeholder="Основной R талант"/>
                            <AddInput :value="character.talents[1]" tag='talent-2' placeholder="Альтернативный T талант"/>
                        </div>
                        <AddInput :value="character.description" text="Описание" tag='description' variant="area"/>
                        <AbilityInputs v-for="index in abilityInputs" :ability="character.abilities[index-1]" :key="index" :index="index" />
...
import { PropType } from 'vue'
    ...
    export default defineNuxtComponent({
        ...
        data() {
            return {
                abilityInputs: this.character?.abilities.length||1
            }
        },
        mounted() {
